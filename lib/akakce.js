...
function buildAkakceXml(products, options) {
  ...
  const rows = analysis.eligible.map((product) => {
    ...
    const name = String(product.name || "").trim().slice(0, AKAKCE_NAME_MAX);
    const pageUrl = productPageUrl(product, siteBaseUrl).slice(0, AKAKCE_URL_MAX);
    ...
    const brand = String(product.brand || "").trim().slice(0, AKAKCE_BRAND_MAX);
    ...
    parts.push("<shipPrice>" + formatAkakceDecimal(settings.shipPrice, 2) + "</shipPrice>");
