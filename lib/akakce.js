...
function analyzeAkakceProducts(products, options) {
  ...
  for (const product of products || []) {
    ...
    const imageUrl = mirrorIndex
      ? resolveFeedImageUrl(product.image, siteBaseUrl, mirrorIndex)
      : absoluteUrl(product.image, siteBaseUrl);
...
function analyzeSupplierFeedIssues(product, options) {
  ...
  if (analysis.eligible.length) return [];
  return (analysis.excluded[0] && analysis.excluded[0].reasons) || ["Feed uygun değil"];
}
...
function productPageUrl(product, siteBaseUrl) {
  const base = String(siteBaseUrl || "").replace(/\/+$/, "");
  const id = encodeURIComponent(String(product.id || product.supplierSku || "").trim());
  const path = "/urun-detay?id=" + id;
  ...
  const rows = analysis.eligible.map((product) => {
    ...
    const images = Array.isArray(product.images)
      ...
      : product.image
        ? [product.image]
        : [];
    const small = mirrorIndex
      ? resolveFeedImageUrl(images[0] || product.image, siteBaseUrl, mirrorIndex)
      : absoluteUrl(images[0] || product.image, siteBaseUrl);
    const large = mirrorIndex
      ? resolveFeedImageUrl(images[1] || images[0] || product.image, siteBaseUrl, mirrorIndex)
      : absoluteUrl(images[1] || images[0] || product.image, siteBaseUrl);
    if (mirrorIndex && (!small || !large)) return "";
