...
function buildTreeFromSupplierProducts(products) {
  ...
  for (const item of products || []) {
    const parentName = cleanCategoryName(item.mainCategory);
    const childName =
      cleanCategoryName(item.midCategory) || cleanCategoryName(item.subCategory) || "Genel";
...
function suggestSiteCategory(product, categories) {
  const parentName = cleanCategoryName(product && product.mainCategory);
  const childName =
    cleanCategoryName(product && product.midCategory) ||
    cleanCategoryName(product && product.subCategory) ||
    "Genel";
