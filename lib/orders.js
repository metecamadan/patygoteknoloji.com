...
/** Panelden elle seçilebilir — ödeme durumları banka callback’ine aittir. */
const ADMIN_FULFILLMENT_STATUSES = new Set(["preparing", "cancelled", "refunded"]);

const MAX_PAYMENT_EVENTS = 40;
...
function rowToOrder(db, row, preloadedItems) {
  ...
  try {
    const rawEvents = row.payment_events_json ? JSON.parse(row.payment_events_json) : null;
    paymentEvents = Array.isArray(rawEvents) ? rawEvents : [];
  } catch (_) {
    paymentEvents = [];
  }
  return {
...
function createOrderStore(root) {
    ...
    const tx = db.transaction(() => {
      db.prepare(
        ...
        ON CONFLICT(id) DO UPDATE SET
          ...
          payment_events_json=excluded.payment_events_json,
        ...
        Array.isArray(order.paymentEvents) && order.paymentEvents.length
          ? JSON.stringify(order.paymentEvents.slice(-MAX_PAYMENT_EVENTS))
          : null,
  ...
  /**
   * Banka callback kaydı: append-only olay + latest bankResponse.
   * decision: { applyStatus, paid, outcome } from decidePaymentFromBank
   */
  function recordBankCallback(orderId, event, decision) {
    const current = get(orderId);
    if (!current) return null;
    const at = String((event && event.at) || new Date().toISOString());
    const row = Object.assign({}, event || {}, {
      at,
      outcome: String((decision && decision.outcome) || event.outcome || "recorded").slice(0, 64),
    });
    const events = Array.isArray(current.paymentEvents) ? current.paymentEvents.slice() : [];
    events.push(row);
    const patch = {
      bankResponse: row,
      paymentEvents: events.slice(-MAX_PAYMENT_EVENTS),
    };
    const alreadyPaid =
      Boolean(current.paymentTaken) ||
      current.paymentStatus === "paid" ||
      current.status === "paid";
    if (decision && decision.applyStatus) {
      if (decision.paid) {
        ...
        if (current.status === "payment_pending" || current.status === "payment_failed") {
          patch.status = "payment_failed";
        }
      }
    } else if (alreadyPaid) {
      // Kanıt güncellenir; ödeme statüsü korunur
      patch.paymentStatus = "paid";
      patch.paymentTaken = true;
    }
    return update(orderId, patch);
  }

  /** bankResponse’ta başarılı kanıt varken failed kalan siparişleri düzeltir. */
  function reconcilePaidFromBankEvidence(orderId) {
    const current = get(orderId);
    if (!current) return null;
    const br = current.bankResponse || {};
    const codeOk = String(br.responseCode || "") === "VPS-0000";
    const hashOk = br.hashOk === true;
    const amountOk = br.amountOk === true;
    const alreadyPaid =
      Boolean(current.paymentTaken) ||
      current.paymentStatus === "paid" ||
      current.status === "paid";
    if (!codeOk || !hashOk || !amountOk || alreadyPaid) return current;
    return update(orderId, {
      paymentStatus: "paid",
      paymentTaken: true,
      status:
        current.status === "payment_failed" || current.status === "payment_pending"
          ? "paid"
          : current.status,
      bankResponse: Object.assign({}, br, {
        reconciledAt: new Date().toISOString(),
        reconcileReason: "bank_evidence_success",
      }),
    });
  }
  ...
  function listCustomers(options) {
    ...
    return db
      .prepare(
        `SELECT id, full_name AS fullName, company, email, phone, tax_id AS taxId,
...
module.exports = { createOrderStore, ORDER_STATUSES, ADMIN_FULFILLMENT_STATUSES };
