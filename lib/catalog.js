...
function publicImages(product, limit) {
  ...
  const images = Array.isArray(product.images)
    ...
    : product.image
      ? [product.image]
      : [];
  return filterSupplierGalleryImages(images, cap);
}
...
function toPublicProduct(product, options) {
  ...
  const images = publicImages(enriched, compact ? 2 : 10);
  const out = {
    id: enriched.id,
...
function buildStorefrontIndex(products) {
  ...
  const compactAll = (products || []).map((item) =>
    attachProductUrlFields(toPublicProduct(item, { compact: true }), routeIndex)
  );
...
function queryPublicCatalog(products, params, options) {
  ...
  if (filtered.mode === "single") {
    const pub = filtered.item
      ? attachProductUrlFields(toPublicProduct(filtered.item), routeIndex)
      : null;
    return {
      products: pub ? [pub] : [],
      total: pub ? 1 : 0,
  ...
  if (filtered.mode === "multi") {
    const found = withPublicProductUrls(
      (filtered.list || []).map(toPublicProduct),
      routeIndex
    );
    return {
      products: found,
