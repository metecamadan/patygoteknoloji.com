...
function queryPublicCatalog(products, params) {
  ...
  if (id) {
    ...
    const pub = one ? toPublicProduct(one) : null;
    return {
      products: pub ? [pub] : [],
      total: pub ? 1 : 0,
      page: 1,
  ...
  if (ids.length) {
    ...
    const found = list.filter((item) => item && want.has(item.id)).map(toPublicProduct);
    return {
      products: found,
      total: found.length,
      page: 1,
  ...
  if (parent) list = list.filter((item) => siteParentOf(item) === parent);
  ...
  list = applyFacetFilters(list, query);
  ...
  return {
    products: list.slice(start, start + limit).map((item) => toPublicProduct(item, { compact: true })),
    total,
    page: safePage,
    limit,
