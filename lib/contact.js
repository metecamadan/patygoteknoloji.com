...
async function deliverSimpleMail(payload, options) {
  ...
  const requestedTo = String((payload && payload.to) || "").trim();
  const fallback = String(env.CONTACT_TO || CONTACT_TO).trim() || CONTACT_TO;
  const to = isValidEmail(requestedTo) ? requestedTo : fallback;
  if (!isValidEmail(to)) throw new Error("Geçerli bir alıcı e-posta gerekli.");
  ...
  const res = await fetchFn("https://formsubmit.co/ajax/" + encodeURIComponent(to), {
    ...
    headers: {
      ...
      Accept: "application/json",
      Origin: "https://patygoteknoloji.com",
      Referer: "https://patygoteknoloji.com/admin",
    },
  ...
  if (!res.ok || json.success === "false" || json.success === false) {
    if (/activation/i.test(message)) {
      throw new Error(
        "FormSubmit aktivasyon bekliyor: " + to + " kutusundaki Activate Form linkine tıklayın."
      );
    }
    throw new Error(message || "E-posta gönderimi başarısız.");
  }
