...
function resolveStorefrontProductImages(product, options) {
  ...
  for (const url of raw) {
    ...
    if (entry && opts.dataRoot && !mirrorEntryIsUsable(entry, opts.dataRoot, opts.placeholderMirrorFiles)) continue;
    ...
    if (opts.dataRoot && entry && !mirrorEntryIsUsable(entry, opts.dataRoot, opts.placeholderMirrorFiles)) continue;
    seen.add(resolved);
    out.push(resolved);
    if (out.length >= cap) break;
  }
