...
function resolveStorefrontProductImages(product, options) {
  ...
  const placeholderMirrorFiles = opts.placeholderMirrorFiles;
  ...
  for (const url of raw) {
    ...
    if (entry && opts.dataRoot && !mirrorEntryIsUsable(entry, opts.dataRoot, placeholderMirrorFiles)) {
      continue;
    }
    ...
    if (opts.dataRoot && entry && !mirrorEntryIsUsable(entry, opts.dataRoot, placeholderMirrorFiles)) {
      continue;
    }
    seen.add(resolved);
    out.push(resolved);
