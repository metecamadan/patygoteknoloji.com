...
function buildSalesInvoicePayload(order, env) {
  ...
  const subtotal =
    Number(order.subtotal) || details.reduce((sum, row) => sum + Number(String(row.net).replace(/,/g, "")), 0);
  const vat =
    Number(order.vat) || details.reduce((sum, row) => sum + Number(String(row.tax).replace(/,/g, "")), 0);
...
async function fetchInvoicePdfAttachment(pdfUrl, options) {
  const opts = options || {};
  ...
  try {
    ...
    return {
      filename: String(opts.filename || "fatura.pdf").slice(0, 180),
      content: buf,
...
async function cancelSalesInvoice(guid, options) {
  ...
  const data = await parseBizimHesapResponse(res);
  return {
    cancelled: String(data.status || "") === "0" || !String(data.error || "").trim(),
    status: data.status,
    error: data.error || "",
  };
}
