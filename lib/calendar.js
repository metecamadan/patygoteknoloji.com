...
function normalizeNotifyEmail(input, type, existing) {
  const hasInput = input && Object.prototype.hasOwnProperty.call(input, "notifyEmail");
  const raw = hasInput
    ? String(input.notifyEmail || "").trim().slice(0, 120)
    : existing && existing.notifyEmail
      ? String(existing.notifyEmail).trim()
      : "";
  if (type === "reminder") {
    if (!isValidEmail(raw)) throw new Error("Hatırlatıcı için geçerli bir e-posta gerekli.");
    return raw.toLowerCase();
  }
  return isValidEmail(raw) ? raw.toLowerCase() : null;
}
    ...
    const done =
      input && Object.prototype.hasOwnProperty.call(input, "done")
        ...
        : existing
          ...
          : false;
    const notifyEmail = normalizeNotifyEmail(input, type, existing);
