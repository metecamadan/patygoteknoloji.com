...
const RETIRED_PARENT_SLUGS = new Set(["bilgisayar-tablet", "cevre-birimleri"]);
...
function cloneCategoryTree(tree) {
  return (Array.isArray(tree) ? tree : []).map((parent) => ({
    slug: parent.slug,
    name: parent.name,
  ...
  function read() {
    try {
      ...
      if (list.length !== rawLen) {
        writeCategoriesFile(file, { version: 1, categories: list });
      }
      return list;
    } catch (_) {
      return seed();
