...
function extractProductId(pathValue, explicitId, options) {
  ...
  try {
    ...
    if (/urun-detay/i.test(url.pathname)) {
      return String(url.searchParams.get("id") || "")
        .trim()
        .slice(0, 80);
    }
    const resolver = options && options.resolveProductIdFromPath;
    if (typeof resolver === "function") {
      const fromPath = resolver(url.pathname);
      if (fromPath) return String(fromPath).trim().slice(0, 80);
    }
    return "";
  } catch (_) {
    return "";
...
function createAnalyticsStore(root, options) {
  ...
  const extractOptions = {
    resolveProductIdFromPath: settings.resolveProductIdFromPath,
  };
  ...
  function record(event) {
    ...
    if (event.type === "page_view") {
      ...
      const productId = extractProductId(fullPath, event.productId, extractOptions);
      if (productId) {
  ...
  function productViewCounts(requestedDaysOrRange) {
    const range = resolvePeriodRange(requestedDaysOrRange, now());
    const data = read();
    const current = aggregate(data, range.from, range.to);
    return current.productViews || {};
  }

  return { record, summary, productViewCounts };
}
