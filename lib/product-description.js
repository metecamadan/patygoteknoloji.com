...
function needsGeneratedDetails(details, name) {
  ...
  if (!nameKey || !detailKey) return true;
  if (nameKey === detailKey) return true;
  const shorter = nameKey.length <= detailKey.length ? nameKey : detailKey;
  const longer = shorter === nameKey ? detailKey : nameKey;
  if (longer.includes(shorter) && longer.length - shorter.length <= 24) return true;
  return false;
}
