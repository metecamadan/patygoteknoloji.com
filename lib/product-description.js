...
const { parseProductSpecChips } = require("./product-detail-specs");
...
function enrichProductCopy(product, options) {
  ...
  if (!skipDetails && !isUsableCopy(out.details, name)) {
...
function needsGeneratedCopy(product) {
  const name = String(product.name || "");
  return !isUsableCopy(product.description, name) || !isUsableCopy(product.details, name);
}
