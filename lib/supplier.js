...
const { filterSupplierGalleryImages } = require("./product-images");
const { readJsonFileInWorker } = require("./read-json-async");
...
function isAllowedRedirectHost(hostname, allowedHosts, originHost) {
  ...
  if (isBlockedFeedHost(host)) return false;
...
async function fetchSupplierXml(url, options) {
  ...
  if (isBlockedFeedHost(current.hostname)) {
    throw new Error("Bu tedarikçi alan adına izin verilmemiş.");
  }
  let dispatcher;
  try {
    ...
    for (let hop = 0; hop <= maxRedirects; hop += 1) {
      ...
      if ([301, 302, 303, 307, 308].includes(status)) {
        ...
        const { assertPublicHost } = require("./network-guard");
        await assertPublicHost(next.hostname, settings.resolveHost);
        current = next;
        continue;
      }
      if (!response.ok) {
...
function stockQtyFromRecord(record) {
...
function collectSupplierImages(record, baseUrl) {
  ...
  return filterSupplierGalleryImages(images, MAX_SUPPLIER_IMAGES);
}
...
function normalizeSupplierRecord(record, index, baseUrl) {
  ...
  const stockQty = stockQtyFromRecord(record);
...
function isBlockedFeedHost(hostname) {
  ...
  if (host === "avansas.com" || host.endsWith(".avansas.com")) return true;
  return false;
}
...
function createSupplierStore(root, options) {
  ...
  function isFeedHostAllowed(rawUrl) {
    let host = "";
    try {
    ...
    if (isBlockedFeedHost(host)) return false;
    const hosts = (options.allowedHosts || []).map((item) => String(item).toLowerCase());
  ...
  function getConfig() {
    ...
    let url = String(saved.url || "").trim();
    if (!url) {
      const envUrl = String(options.envUrl || "").trim();
      if (envUrl && isFeedHostAllowed(envUrl)) url = envUrl;
    } else if (!isFeedHostAllowed(url)) {
      url = "";
    }
    return {
      url,
  ...
  function dropDisallowedFeed() {
    const saved = readJson(files.config, {});
    const url = String(saved.url || options.envUrl || "").trim();
    if (!url || isFeedHostAllowed(url)) return;
    atomicWriteJson(files.config, {
      url: "",
      name: String(options.defaultName || "XML Kaynağı").trim().slice(0, 60),
      savedAt: new Date().toISOString(),
    });
    writeCache([]);
    try {
      fs.unlinkSync(files.lastXml);
    } catch (_) {}
  }
  ...
  function getSettings() {
    ...
    return {
      ...
      criticalStockQty: Number.isFinite(Number(saved.criticalStockQty))
        ...
        : 0,
      scheduleMode: saved.scheduleMode === "interval" ? "interval" : "fixed",
  ...
  function saveSettings(patch) {
    ...
    listedProductsMemo = null;
    listedProductsStamp = "";
    return next;
  }

  let cacheMemo = null;
  let cacheMemoMtime = 0;
  let cacheById = null;
  let overridesMemo = null;
  let overridesMemoMtime = 0;
  let listedProductsMemo = null;
  let listedProductsStamp = "";
  ...
  function getOverrides() {
    const mtime = fileMtime(files.overrides);
    if (overridesMemo && mtime === overridesMemoMtime) return overridesMemo;
    const value = readJson(files.overrides, {});
    overridesMemo = value && typeof value === "object" && !Array.isArray(value) ? value : {};
    overridesMemoMtime = mtime;
    return overridesMemo;
  }

  function getCache() {
    const mtime = fileMtime(files.cache);
    if (cacheMemo && mtime === cacheMemoMtime) return cacheMemo;
    const value = readJson(files.cache, []);
    cacheMemo = Array.isArray(value) ? value : [];
    cacheMemoMtime = mtime;
    cacheById = null;
    listedProductsMemo = null;
    listedProductsStamp = "";
    return cacheMemo;
  ...
  async function preloadCacheAsync() {
    const mtime = fileMtime(files.cache);
    if (cacheMemo && mtime === cacheMemoMtime) return cacheMemo;
    ...
    try {
      const value = await readJsonFileInWorker(files.cache);
      const nextMtime = fileMtime(files.cache);
      cacheMemo = Array.isArray(value) ? value : [];
      cacheMemoMtime = nextMtime;
      cacheById = null;
      return cacheMemo;
    } catch (_) {
      return getCache();
  ...
  function cacheIndex() {
    ...
    const index = new Map();
    cache.forEach((row) => {
      if (row && row.id) index.set(String(row.id), row);
    });
    cacheById = index;
    return index;
  }
  ...
  function writeCache(products) {
    cacheMemo = Array.isArray(products) ? products : [];
    cacheById = null;
    listedProductsMemo = null;
    listedProductsStamp = "";
    atomicWriteJson(files.cache, cacheMemo);
    cacheMemoMtime = fileMtime(files.cache);
  }

  function writeOverrides(value) {
    overridesMemo =
      value && typeof value === "object" && !Array.isArray(value) ? value : {};
    atomicWriteJson(files.overrides, overridesMemo);
    overridesMemoMtime = fileMtime(files.overrides);
    listedProductsMemo = null;
    listedProductsStamp = "";
  }
  ...
  function getDisplayName() {
    return getConfig().name || options.defaultName || "XML Kaynağı";
  }

  function status() {
    const config = getConfig();
    const settings = getSettings();
    // Never JSON.parse the full supplier cache on the status path — that blocks every panel API.
    let itemCount = Number(settings.itemCount);
    if (!Number.isFinite(itemCount) || itemCount < 0) {
      itemCount = cacheMemo && Array.isArray(cacheMemo) ? cacheMemo.length : 0;
    }
    const catalogStale =
      Boolean(settings.catalogStale) ||
      (settings.lastFetchStatus === "error" && itemCount > 0);
    const minutes = resolveScheduleMinutes(settings);
    const schedule = scheduleSummary(
      minutes,
      settings.scheduleMode === "interval" ? settings.scheduleIntervalMinutes : null
    );
  ...
  function setSettings(patch) {
    ...
    if (patch && patch.scheduleMode === "fixed") {
      next.scheduleMode = "fixed";
    }
    const allowInterval = patch && patch.scheduleMode === "interval";
    if (allowInterval && (patch.scheduleStartMinute !== undefined || patch.scheduleStart !== undefined)) {
      const start =
    ...
    if (allowInterval && patch.scheduleIntervalMinutes !== undefined) {
  ...
  function updateOverrides(updates) {
    ...
    for (const update of updates || []) {
      ...
      assignTextOverride(next, "siteMid", update.siteMid, 80);
      assignTextOverride(next, "siteChild", update.siteChild, 80);
      assignTextOverride(next, "urlSlug", update.urlSlug, 120);
      assignTextOverride(next, "urlCategorySegment", update.urlCategorySegment, 40);
      if (next.active === true) {
        const cats = loadCategories();
        if (
          !hasSiteCategory(
            { siteParent: next.siteParent, siteMid: next.siteMid, siteChild: next.siteChild },
            cats
          )
        ) {
  ...
  function hydrateListedProduct(item, settings, overrides, categories, rates, catalogStale) {
    ...
    const listed = Object.assign({}, item, {
      ...
      siteMid: pickOverrideText(override, "siteMid", ""),
      siteChild: pickOverrideText(override, "siteChild", ""),
      urlSlug: pickOverrideText(override, "urlSlug", ""),
      urlCategorySegment: pickOverrideText(override, "urlCategorySegment", ""),
    ...
    const enriched = enrichProductCopy(listed, {
      skipDescription: Boolean(override.description),
      skipDetails: Boolean(override.details),
    });
    listed.description = enriched.description;
    listed.details = enriched.details;
    return listed;
  }

  function listedStamp() {
    return [
      fileMtime(files.cache),
      fileMtime(files.overrides),
      fileMtime(files.settings),
    ].join(":");
  }

  function listProducts() {
    const stamp = listedStamp();
    if (listedProductsMemo && stamp === listedProductsStamp) return listedProductsMemo;
    const settings = getSettings();
    ...
    const listed = cache.map((item) =>
      hydrateListedProduct(item, settings, overrides, categories, rates, catalogStale)
    );
    listedProductsMemo = listed;
    listedProductsStamp = stamp;
    return listed;
  }

  async function preloadListedProductsAsync() {
    await preloadCacheAsync();
    if (listedProductsMemo && listedStamp() === listedProductsStamp) return listedProductsMemo;
    const settings = getSettings();
    const overrides = getOverrides();
    const cache = getCache();
    const catalogStale =
    ...
    for (let i = 0; i < cache.length; i += 1) {
      ...
      listed.push(
        hydrateListedProduct(cache[i], settings, overrides, categories, rates, catalogStale)
      );
    }
    listedProductsMemo = listed;
    listedProductsStamp = listedStamp();
    return listed;
  }
  ...
  function getProductById(id) {
    ...
    const item = cacheIndex().get(wanted);
    if (!item) return null;
    const settings = getSettings();
    const overrides = getOverrides();
    const cache = getCache();
    const catalogStale =
      Boolean(settings.catalogStale) ||
      (settings.lastFetchStatus === "error" && cache.length > 0);
    return hydrateListedProduct(
      item,
      settings,
      overrides,
      loadCategories(),
      readCachedRates(root),
      catalogStale
    );
  }
