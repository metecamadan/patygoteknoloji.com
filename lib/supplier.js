...
function normalizeSupplierRecord(record, index, baseUrl) {
  ...
  const stockQty = parseStockQty(stockRaw);
...
function createSupplierStore(root, options) {
  ...
  function updateOverrides(updates) {
    ...
    for (const update of updates || []) {
      ...
      if (update.image === null || update.image === "") delete next.image;
  ...
  function listProducts() {
    ...
    return cache.map((item) => {
      ...
      const listed = Object.assign({}, item, {
        ...
        name: pickOverrideText(override, "name", item.name),
        ...
        mainCategory: pickOverrideText(override, "mainCategory", item.mainCategory),
        midCategory: pickOverrideText(override, "midCategory", item.midCategory),
        subCategory: pickOverrideText(override, "subCategory", item.subCategory),
        description: pickOverrideText(override, "description", item.description),
        specialCode: pickOverrideText(override, "specialCode", item.specialCode),
        currency: pickOverrideText(override, "currency", item.currency),
        unit: pickOverrideText(override, "unit", item.unit),
        image: pickOverrideText(override, "image", item.image),
