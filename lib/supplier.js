...
async function validateSupplierUrl(rawUrl, allowedHosts) {
  let url;
  try {
...
async function fetchLimited(url) {
  ...
  try {
    ...
    if (length > MAX_XML_BYTES) throw new Error("XML dosyası 15 MB sınırını aşıyor.");
    ...
    while (true) {
      ...
      if (size > MAX_XML_BYTES) {
        await reader.cancel();
        throw new Error("XML dosyası 15 MB sınırını aşıyor.");
      }
...
function createSupplierStore(root, options) {
  ...
  const files = {
    config: path.join(runtimeDir, "supplier-config.json"),
    cache: path.join(runtimeDir, "supplier-cache.json"),
    overrides: path.join(runtimeDir, "supplier-overrides.json"),
    settings: path.join(runtimeDir, "supplier-settings.json"),
  };
  ...
  async function saveUrl(rawUrl) {
    const url = await validateSupplierUrl(rawUrl, options.allowedHosts);
    atomicWriteJson(files.config, { url: url.href, savedAt: new Date().toISOString() });
    return { configured: true, maskedUrl: maskUrl(url.href), host: url.hostname };
  }
  ...
  async function refresh() {
    ...
    try {
      const url = await validateSupplierUrl(config.url, options.allowedHosts);
      const xml = await fetchLimited(url);
