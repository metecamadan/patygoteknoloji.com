...
function normalizeSupplierRecord(record, index, baseUrl) {
  ...
  const mainCategory = field(record, ["mainCategory", "anaKategori", "AnaKategori"]);
  const midCategory = field(record, ["midCategory", "araKategori", "AraKategori"]);
  const subCategory = field(record, ["subCategory", "altKategori", "AltKategori"]);
  const vatPercent = parseNumber(field(record, ["kdv", "KDV", "vat", "vatRate"])) || 20;
  const currency =
    field(record, ["currency", "dovizTuru", "DovizTuru", "paraBirimi"]) || "TRY";
